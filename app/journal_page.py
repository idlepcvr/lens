...
function openTrade(id){
  ...
  $('modal').innerHTML=`<div class="bm-bg open" id="mbg"><div class="bm ${isL?'long':'short'}">
    ...
    <div id="bm-chart"></div>
    <div style="margin:-6px 0 12px"><a href="/chart-review?trade=${t.id}" style="font-size:11px;color:var(--accent);text-decoration:none;font-family:var(--mono)">Full chart — SMA/Bollinger/RSI/MACD/levels →</a></div>
    <div class="bm-cols">
      <div class="bm-grid">
  ...
  // chart — small preview only. Indicators/RSI/MACD/levels moved to their
  // own full-size page (/chart-review): the modal had no room to show them
  // without squashing every pane unreadable.
  const el=$('bm-chart');
