...
// ── big modal ────────────────────────────────────────────────────────────────
// Quick-glance popup for grading without leaving the list — direct ask:
...
async function openTrade(id){
  ...
  $('modal').innerHTML=`<div class="bm-bg open" id="mbg"><div class="bm ${isL?'long':'short'}">
    ...
    <div id="bm-chart"></div>
    <div style="margin:-6px 0 12px"><a href="/chart-review?trade=${t.id}&book=${BOOK}" style="font-size:11px;color:var(--accent);text-decoration:none;font-family:var(--mono)">Full chart — SMA/Bollinger/RSI/MACD/levels →</a></div>
    <div id="bm-veto"></div>
    <div class="bm-cols">
      <div class="bm-grid">
  ...
  // veto override — his typed reason for taking this trade against the
  // scanner, if it was one. Was recorded but never linked to the trade
  // until 2026-08-27 (database._link_veto_override) — surfacing it now
  // that it's actually findable.
  fetch('/api/veto-overrides/for-trade?trade_id='+t.id).then(r=>r.json()).then(d=>{
    if(!d.override) return;
    const o=d.override;
    $('bm-veto').innerHTML=`<div class="auto-take" style="border-left-color:var(--accent);margin-bottom:12px">
      <b style="color:var(--accent)">⚠ Taken against the scanner</b>
      ${o.veto_reasons&&o.veto_reasons.length?` · scanner said: <span class="dim">${o.veto_reasons.join(', ')}</span>`:''}
      <div style="margin-top:5px">${(o.user_reason||'').replace(/</g,'&lt;')}</div>
    </div>`;
  }).catch(()=>{});
  // chart — small preview only. Indicators/RSI/MACD/levels are the full
